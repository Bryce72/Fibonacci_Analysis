...
ull fibonacci_dp(int n, ull *ops)
{
    ...
    (*ops)++; //want to put ops here to see how many times this recursive function will trigger.
...
//hoping this will fix my recursion issues
double time_function_recursion(int n, ull *ops)
{
        // Setup timers
    struct timespec begin, end;
    // Get the time before we start
...
double time_function_dp(int n, ull *ops)
{
        // Setup timers
    struct timespec begin, end;
    // Get the time before we start
...
{    
    ...
    case 1:
        ...
        ops = 0;
        time = time_function_recursion(n, &ops);
        printf("time: %f(%llu)\n", time, ops);
        break;
    case 2:
        printf("dynamic programming version\n");
        ops = 0;
        time = time_function_dp(n, &ops);
        printf("time: %f(%llu)\n", time, ops);
        break;
    case 4:
        ops = 0;
